...
def build_live_prediction_features(
    ...
    if forecast_row is not None:
        rainfall_lag_1 = float(pd.to_numeric(pd.Series([forecast_row.get("rainfall_lag_1", np.nan)]), errors="coerce").fillna(0.0).iloc[0])
        rainfall_lag_2 = float(pd.to_numeric(pd.Series([forecast_row.get("rainfall_lag_2", np.nan)]), errors="coerce").fillna(0.0).iloc[0])
        rainfall_lag_3 = float(pd.to_numeric(pd.Series([forecast_row.get("rainfall_lag_3", np.nan)]), errors="coerce").fillna(0.0).iloc[0])

        rh_lag_1 = float(pd.to_numeric(pd.Series([forecast_row.get("relative_humidity_lag_1", np.nan)]), errors="coerce").fillna(0.0).iloc[0])
        rh_lag_2 = float(pd.to_numeric(pd.Series([forecast_row.get("relative_humidity_lag_2", np.nan)]), errors="coerce").fillna(0.0).iloc[0])
        rh_lag_3 = float(pd.to_numeric(pd.Series([forecast_row.get("relative_humidity_lag_3", np.nan)]), errors="coerce").fillna(0.0).iloc[0])

        temp_lag_1 = float(pd.to_numeric(pd.Series([forecast_row.get("temp_mid_lag_1", np.nan)]), errors="coerce").fillna(0.0).iloc[0])
        temp_lag_2 = float(pd.to_numeric(pd.Series([forecast_row.get("temp_mid_lag_2", np.nan)]), errors="coerce").fillna(0.0).iloc[0])
        temp_lag_3 = float(pd.to_numeric(pd.Series([forecast_row.get("temp_mid_lag_3", np.nan)]), errors="coerce").fillna(0.0).iloc[0])
    else:
...
with tab5:
    ...
    st.info(
        """
This section helps estimate whether a selected **year-month** is likely to be an **outbreak** or **non-outbreak** month.

To use this section:
- Select the **year** and **month** you want to evaluate.
- Enter the climate values for that exact target month.
- Enter the dengue case counts from the previous 3 months.
- Click **Predict**.
...
Choose the exact year and month you want the system to evaluate, such as February 2027.

**2. Rainfall**  
This should represent the rainfall for the target month itself.  
For example, if you are predicting February 2027, enter the rainfall for February 2027, not January.

**3. Relative Humidity**  
This should represent the average humidity for the target month itself.

**4. Temperature**  
This should represent the average temperature for the target month itself.

**5. Cases Last Month / 2 Months Ago / 3 Months Ago**  
These are the dengue case counts from the three months before the target month.  
Example: if you predict February 2027, these should be January 2027, December 2026, and November 2026.

### Important Reminder
You do **not** need to enter lagged climate values, rolling averages, or seasonal encoded values manually.  
The dashboard computes those automatically from the selected year-month and the forecast/profile data.
...
"""
    ...
    if model is None:
        ...
        with select_col1:
            selected_year_num = st.selectbox(
                "Select Year",
                year_options,
                index=0
            )
        ...
        if target_forecast_row is not None:
            rainfall_default = float(target_forecast_row.get("rainfall", get_profile_value(selected_month_num, "rainfall", month_profile, monthly, 0.0)))
            humidity_default = float(target_forecast_row.get("relative_humidity", get_profile_value(selected_month_num, "relative_humidity", month_profile, monthly, 0.0)))
            temp_default = float(target_forecast_row.get("temp_mid", get_profile_value(selected_month_num, "temp_mid", month_profile, monthly, 0.0)))
        else:
        ...
        st.info(
            f"You are preparing inputs for **{month_name_from_number(selected_month_num)} {selected_year_num}**. "
            "Enter the climate values for that exact month. The dashboard will automatically compute the previous-month climate lag variables in the background."
        )
        ...
        target_year_month_text = f"{selected_year_num}-{selected_month_num:02d}"
        st.caption(f"Target prediction period: {target_year_month_text}")
        ...
        if st.button("Predict"):
            ...
            result_col1.success(f"Predicted Class for {month_name_from_number(selected_month_num)} {selected_year_num}: {outbreak_label_from_binary(pred)}")
            result_col2.info(
...
"""
            ...
            if barangay_risk_profile is not None and not barangay_risk_profile.empty:
                ...
                if "Month" in barangay_live.columns:
                    seasonal_subset = barangay_live[barangay_live["Month"] == selected_month_num].copy()
                ...
                display_barangay_live_top3 = round_display_columns(
                    barangay_live_top3[keep_cols],
                ...
                st.dataframe(display_barangay_live_top3, use_container_width=True)

                fig_live_barangay = px.bar(
                    round_display_columns(barangay_live_top3, ["predicted_barangay_cases_proxy"], decimals=2),
                    x="Barangay",
                    y="predicted_barangay_cases_proxy",
                    color="Barangay",
                    text="predicted_barangay_cases_proxy",
                    title=f"Top 3 Likely Barangays for {month_name_from_number(selected_month_num)} {selected_year_num}"
                )
                ...
                st.caption("Note: predicted_city_cases_proxy and predicted_barangay_cases_proxy are weighted proxy estimates, so decimals are expected. They are not exact confirmed case counts.")
            else:
